            ...
            if videofile.endswith(".mp4"):
                ...
                while is_success:
                    if frame_number in new_frame_numbers:
                        ...
                        video_name = os.path.splitext(videofile)[0]  # get the video name
                        filename = 'frame_{}_snippet_{}_{}.jpg'.format(new_frame_num, snippet_num, video_name)
                        
                        
                        # add another directory level for the video name
                        save_path_and_name = os.path.join(output_folder, traintest, subfolder, video_name, filename)
...
def main():
    ...
    testfiles = set()
    trainvalfiles = set()
    with open('statistics-with-locations.csv') as csv_file:
